...
# Common English words that regex-based extraction can mistakenly capture as
# identifiers when a keyword (interface/api/function/...) is followed by prose
# instead of a real symbol name, e.g. "this interface is used to ..." or
# "the API documentation describes ...".
STOPWORDS = {
    "a", "an", "the", "this", "that", "these", "those", "it", "its",
    "to", "of", "in", "on", "for", "and", "or", "but", "with", "is",
    "are", "was", "were", "be", "been", "being", "as", "at", "by",
    "from", "into", "onto", "over", "under", "not", "no", "if", "then",
    "than", "so", "such", "which", "who", "whom", "whose", "when",
    "where", "why", "how", "all", "any", "each", "some", "most", "more",
    "other", "own", "same", "you", "your", "we", "our", "they", "their",
    "documentation", "elements",
}
# "interface" is only a real declaration keyword in these languages; in C/C++
# comments and prose it is common English and produces false Class matches
# such as "interface is", "interface for", "interface with".
INTERFACE_KEYWORD_SUFFIXES = {".ts", ".tsx", ".java", ".kt"}
...
def extract_generic_code(path: Path, text: str, extraction: Extraction) -> None:
    ...
    module = add_file_entity(extraction, path)
    extraction.code_entities[source].append(module)
    class_keywords = "class|struct" + ("|interface" if path.suffix.lower() in INTERFACE_KEYWORD_SUFFIXES else "")
    patterns = [
        ("Class", re.compile(rf"\b(?:{class_keywords})\s+([A-Za-z_]\w*)")),
...
def extract_document(path: Path, text: str, extraction: Extraction) -> None:
    ...
    for match in re.finditer(r"(?i)\b(?:api|endpoint|interface|function)\s*[:`]*\s*([A-Za-z_]\w*)", text):
        name = match.group(1)
        if name == "_" or len(name) < 2 or name.casefold() in STOPWORDS:
            continue
        line = line_number(text, match.start())
...
def level_three(extraction: Extraction) -> None:
    ...
    for entity in extraction.entities.values():
        if entity.entity_type in {"Function", "Class"} and entity.name.casefold() not in STOPWORDS:
            code_by_name[entity.name.casefold()].append(entity)
    for source, docs in extraction.doc_entities.items():
        modules = [
            entity for entity in extraction.entities.values()
            if entity.entity_type == "Module" and entity.source_file == source
        ]
        document_nodes = [entity for entity in docs if entity.entity_type == "Document"]
        for document in document_nodes:
            for module in modules:
                extraction.add_relation(Relation(
                    ...
                    source,
                    document.line_start,
                    "Documentation and source inventory refer to the same file",
                ))
        ...
        for doc in docs:
            ...
            for name in mentioned_names:
                ...
                for candidate in candidates:
                    ...
                    if doc.entity_type in {"Concept", "API"}:
                        extraction.add_relation(Relation(
                            doc.name,
                            "REFERENCES",
                            candidate.name,
                            0.8 if len(candidates) == 1 else 0.6,
                            source,
                            doc.line_start,
                            "Code symbol name is explicitly mentioned in documentation",
                        ))
        if any(doc.entity_type == "API" for doc in docs):
            for api in (doc for doc in docs if doc.entity_type == "API"):
                if api.name.casefold() in GENERIC_NAMES:
                    continue
                for candidate in code_by_name.get(api.name.casefold(), []):
                    if candidate.entity_type in {"Function", "Class"}:
                        extraction.add_relation(Relation(
                            api.name,
                            "REFERENCES",
                            candidate.name,
                            ...
                            source,
                            api.line_start,
                            "API entity name exactly matches a code symbol",
                        ))
        for concept in (doc for doc in docs if doc.entity_type == "Concept"):
            if concept.name.casefold() in GENERIC_NAMES:
                continue
            for candidate in code_by_name.get(concept.name.casefold(), []):
                if candidate.entity_type in {"Function", "Class"}:
                    extraction.add_relation(Relation(
                        concept.name,
                        "REFERENCES",
                        ...
                        source,
                        concept.line_start,
                        "Concept heading exactly matches a code symbol",
                    ))
    for entity in extraction.entities.values():
...
def resolve_relation_endpoints(extraction: Extraction) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    preferred = {
        ...
        "REFERENCES": ({"Concept", "API", "Document"}, {"Function", "Class", "Module"}),
        "REPRESENTS": ({"Document"}, {"Module"}),
    }
    ...
    for relation in extraction.relations.values():
        ...
        def choose(name: str, allowed: set[str]) -> list[Entity]:
            ...
            best = min(priority.get(item.entity_type, 99) for item in candidates)
            preferred = [item for item in candidates if priority.get(item.entity_type, 99) == best]
            return preferred
